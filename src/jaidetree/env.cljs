(ns jaidetree.env
  (:require [clojure.string :as s]
            [clojure.edn :as edn]
            [jaidetree.utils :refer [read-file]]))
...
(defn wrap-pairs [pairs-str] (str "[" pairs-str "]"))
...
(defn optional
  [key default]
  (let [kw (keyword key)
        value (or (aget js/process.env (name kw)) (get env kw) ::not-found)]
    (if (= value ::not-found)
      (do (js/console.warn (str "Optional: Could not find ENV var " key))
          default)
      value)))
